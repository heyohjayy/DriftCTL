...
def write_audit_log(path: Path, result: ScanResult) -> int:
    ...
    events: list[dict[str, Any]] = [{"event_type": "scan", "scan_id": result.scan_id, "timestamp": result.timestamp, "expected_count": result.expected_count, "live_count": result.live_count, "finding_count": len(result.findings)}]
